...
(async()=>{
  ...
  console.log('プラン変更(unl)', (await post(bill,{event:{type:'PRODUCT_CHANGE',app_user_id:uid,product_id:'pk_std_monthly',new_product_id:'pk_unl_monthly',expiration_at_ms:now+30*864e5,event_timestamp_ms:now+1000}},WH)).j, (await post(auth,{action:'status',token:tok})).j.plan);
  console.log('解約(期限まで有効)', (await post(bill,{event:{type:'CANCELLATION',app_user_id:uid,product_id:'pk_unl_monthly',expiration_at_ms:now+30*864e5,event_timestamp_ms:now+2000}},WH)).j, (await post(auth,{action:'status',token:tok})).j.plan);
  console.log('期限切れ', (await post(bill,{event:{type:'EXPIRATION',app_user_id:uid,product_id:'pk_unl_monthly',expiration_at_ms:now-1,event_timestamp_ms:now+3000}},WH)).j, (await post(auth,{action:'status',token:tok})).j.plan);
  console.log('Google形式の商品ID(light:monthly)', (await post(bill,{event:{type:'RENEWAL',app_user_id:uid,product_id:'pk_light:monthly',expiration_at_ms:now+30*864e5,event_timestamp_ms:now+4000}},WH)).j, (await post(auth,{action:'status',token:tok})).j.plan.plan);
