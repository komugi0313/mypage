...
const srv=http.createServer((q,r)=>{
  ...
  for(const plan of ['std','vip']){
    ...
    await pg.evaluate(()=>localStorage.setItem('pk_deep_'+_ym(),String(DEEP_QUOTA[curPlan()])));
    hdrs.length=0; const r1=await ask('彼との恋愛、この先どうなる？'); await pg.waitForTimeout(1200);
    console.log('   使い切り後の恋愛の質問 →',r1.slice(0,90),'| AIへ送信:',hdrs.length,'| 料金シート表示:',await pg.evaluate(()=>document.getElementById('shPlan').classList.contains('on')));
    hdrs.length=0; const r2=await ask('今日は暑いね'); console.log('   使い切り後の雑談 → 返事あり:',r2.length>5,'| AIへ送信:',hdrs.length);
    hdrs.length=0; const r3=await ask('もう死にたい'); console.log('   使い切り後の深刻な相談 → AIへ送信:',hdrs.length,JSON.stringify(hdrs.slice(-1)));
