...
> ★**最優先で実装してほしい機能**（B2B／スクール運用の要）。この一式はプロの「顧客カルテ」です。
> いまは**端末内（localStorage）のみ**なので、機種変・別端末・ブラウザ変更・履歴削除で**顧客データが消えます**。
> 講師・受講生・複数PCで**同じ顧客データを共有**するには `/mypage-get` `/mypage-put` の実装が前提。
> 検索・並べ替え・タグ付けはフロント実装済み（`c.tags` を含む `bazi_mypage_v2` 全体をそのまま同期すればタグも引き継がれます）。
