/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v235';  /* 鑑定文を「専門用語＋やさしい言い換え」のバランス型に：AI鑑定の書き方ルール(YOMI_RULE)に「用語は消さず、初出でその場に普通の言葉をカッコで必ず添える／五行の金はお金ではない等」を明記。表示側(formatReading)にも安全網＝初出の用語に一度だけ「（＝ふつうの言葉）」を自動付与(plainGloss)＋五行の意味の凡例(金＝お金ではない)を表示。AI・オフライン鑑定の両方に共通で効く。app-proのみ。※v234=相性ひと目の活かし方。 */
