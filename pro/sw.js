/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v234';  /* 相性「ひと目」判定カードにグレードごとの「意味＋活かし方（この2人へのひとことアドバイス）」を追加：心の縁(十干＝性格・価値観)／縁・つながり(十二支＝一緒の居心地)を、色ラベルだけでなく鑑定士がお客様に渡せる一言まで表示(CGRADE_STEM_TAKE/CGRADE_BRANCH_TAKE)。恋愛・相性の他出力(寿ゾーン/桃花/相性オフライン文/compatExtras)は既に助言付きで基準充足を確認。app-proのみ。※v233=大運各行の個別助言。 */
