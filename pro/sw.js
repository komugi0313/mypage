/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v147';  /* 時刻入力まわりの「くわしい設定」見出しから「※ふつうは開かなくてOK」の赤文字注記を削除（見出しは残す）。※v146＝エンジン正確化：①節入り判定を北京時間(UTC+8)基準に統一（tyme4tsの節気は北京時間のため、日本の生時をそのまま比べていたズレを解消。年柱・月柱・順逆・立運・大運の判定のみ。日柱・時柱は真太陽時で不変。影響は節入り直前約1時間生まれの約0.2%のみ、大多数は不変）②立運前の「今の大運」を第一運＝月柱に統一（curDu/monthDuOf/duListWithMonth、鑑定文・空亡・夫婦冲・相性・五行の巡りに反映）③印刷命式表：立運1年未満で大運が10年ずれる不具合を修正（干支ずらし廃止＋0歳＝月柱行）④命式表の年運「今年」を立春で切替。基準ケース1981-06-10 22:28は不変。※v145＝生年月日プルダウン／v144＝AI鑑定文の分量 */
