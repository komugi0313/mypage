/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v143';  /* UI整理：出身地を「くわしい設定」からメイン入力欄（時刻の次・既定=東京）へ移動し、補正方法/恋愛の見方/日付の変わり目は畳んだ詳細に集約（普通は開かなくてよい）。料金ページの申込確認下のくどいサイト説明を削除。トップの命式表画像を現行デザイン（列型・ランダム誕生日）に更新。※v142＝モテ度の移植 */
