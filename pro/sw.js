/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v120';  /* 会局セクションを分かりやすく：冒頭に平易な解説（三合＞方合＞半合）＋あなた専用の喜忌キー（追い風/強すぎ注意の五行）を追加、三合・方合（強い会局）を主役に前面表示、大量の半合（ゆるい）は重複除去のうえ折りたたみに。計算は不変・表示のみ。※v119＝通変星ホイールの数え方注記を実装と一致させる：「地支の本気4」→「地支の代表蔵干（司令）4」（各柱とも司令＝月律分野で数えており本気ではないため。命式表の蔵干注記と統一・計算は不変）。※v118＝要約枠セージ緑＋本文15px。※v117＝要約枠を短い結論に＋箇条書き整形修正／v116＝相性の質問もテーマ別化／v115＝個人のオフラインをテーマ別化＋わかりやすさ3原則 */
