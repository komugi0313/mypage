/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v247';  /* 精査対応：(1)要点カードで五行が喜と忌の両方に出る矛盾を解消(喜の五行は忌に出さない)。(2)吉凶星の読み仮名を全星に正しく統一(KICHI_YOMI拡充＝天乙貴人テンオツキジン等)。(3)上部ツールバーの「相性診断」を紫の塗り＋2番目に配置し一気に飛べるように(メイン機能を明確化)。app-proのみ。※v246=過去の振り返りの文体調整。 */
