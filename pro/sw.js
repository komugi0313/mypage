/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v232';  /* 命式ハイライトの「助言落ち」を一掃：通変個数バッジ(強く出る3つ)でも活かし方・注意を表示／格局(命式の型・好配置)に型ごとの活かし方＝適職・過ごし方・注意を追加(KAKU_ADVICE/KAKU_SUB_ADVICE)／天地徳合は要点だけでなく晦気の距離感アドバイスまで表示。app-proのみ。※v231=通変組み合わせ・三合方合の助言表示。 */
