/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v246';  /* 過去の振り返りの文体調整：逃げ言葉(もし/かもしれません/心当たりがあれば)の多用をやめ、命式を根拠に鑑定士として時期の質を言い切る。確認は章末に一度だけ軽く。会局の年は前向きに言い切る。※v245=  /* 過去の振り返りをバランス調整：細かく断定せず（外れると逆効果）、具体年は印象的な1〜2年に絞り、傾向を大きく1〜2点だけ「もし心当たりがあれば」と柔らかく問いかける2〜4文に。年数検出は3年のまま(AIが取捨選択)。※v244=過去の振り返り強化：pastKuboChongを拡張し空亡・冲・会局(三合/方合)の成立年を各直近3年まで検出。chartSummaryTextに【過去の振り返り】を渡し、鑑定書AIの章立てに独立小見出し「過去の振り返り（当たりの確認）」を追加＝過去に起きやすかったこと（変化・別れ・転職・転機）を問いかけ形で提示。meishi-sheet＋①個人版へバイトミラー。※v243=鑑定書に会局連携。 */
