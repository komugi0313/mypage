/* 四柱推命占い自動鑑定 — オフライン対応 Service Worker（経験者版ドメイン専用）
   ...
   オフライン鑑定に自動で切り替わる。 */
const CACHE = 'shichu-jidou-v242';  /* 命式表の印刷「下端の線が切れる」を根本解決：内容が多い人ほど表が高くA4を超えて下端(時柱の下線)が切り取られていた。#sheetをheight:auto＋overflow:visibleにし、内容量に応じた縮小率(--mfit＝fitPrintSheetが自動計算)でzoomして必ず1枚に収める＝時柱の下線まで必ず印刷。複数命式でPDF検証済み。meishi-sheet＋①個人版へバイトミラー。※v241=大運会局に半会・半方表示。 */
