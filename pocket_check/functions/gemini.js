...
//   前提＝DEEP原価 ¥4.0/通（保守的な上限：巨大なシステムプロンプト＋長い出力を想定）＋雑談等で約+25%。年額契約者（月割で総額が安い）でも60%維持。
//   ★実測でGemini請求額÷通数を確認し、原価が下がっていれば上限を上げてよい（プロンプトキャッシュ導入で入力コストを大幅減できる＝品質を落とさずに上限を戻せる）。
//   → Standard 50／Pro 105／VIP 175／無料はお試し合計15。※Proは100→105：全プランを60%に揃えると1回単価が収束し、上位ほど割高な逆転が出るため微調整（Std¥58>Pro¥56.2>VIP¥56.0）。
const LIMITS = {
  free:     { deep: 15,  total: 60,  ip: 120 },  // 無料お試し：本格鑑定 合計15（3日間・月間枠）＋雑談。獲得コストとして上限厳しめ
  standard: { deep: 50,  total: 400, ip: 500 },  // Standard $19.99：本格鑑定 50/月＋雑談・理屈（原価¥4/通でも利益率≥60%を保証。単価¥58/回）
  pro:      { deep: 105, total: 600, ip: 700 },  // Pro $39.99：本格鑑定 105/月（100→105。上位ほど1回が割高になる逆転を解消。単価¥56.2/回・利益率≥60%）
  vip:      { deep: 175, total: 900, ip: 1000 }, // VIP $69.99：本格鑑定 175/月＋雑談・理屈（単価¥56.0/回・上位が一番お得。利益率≥60%）
};
